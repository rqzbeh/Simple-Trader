...
func TestDynamicCryptoScreenerEvaluation(t *testing.T) {
	...
	t.Run("Rejects candidates on live stats error without fallback numbers", func(t *testing.T) {
		cfg := ScreenerConfig{
			Min24hVolume:   50000000.0,
			MaxSpreadBps:   10.0,
			CandidatePairs: []string{"FAIL/USD"},
		}
		...
		provider := &mockMarketStatsProvider{
			stats: map[string]struct {
				...
				"FAIL/USD": {
					err: fmt.Errorf("exchange network unreachable"),
				},
			},
		}

		screener := NewDynamicCryptoScreener(cfg, provider, nil, nil)
		evaluated := screener.RunScreeningCycle(context.Background())
		if len(evaluated) != 1 {
			t.Fatalf("expected 1 evaluated asset, got %d", len(evaluated))
		}
		if evaluated[0].Status != "DISQUALIFIED" {
			t.Errorf("expected FAIL/USD to be DISQUALIFIED, got %s", evaluated[0].Status)
		}
		if evaluated[0].Price != 0 {
			t.Errorf("expected Price to be 0, got %f", evaluated[0].Price)
		}
		if evaluated[0].BidAskSpreadBps != 0 {
			t.Errorf("expected BidAskSpreadBps to be 0, got %f", evaluated[0].BidAskSpreadBps)
		}
		if evaluated[0].Volume24h != 0 {
			t.Errorf("expected Volume24h to be 0, got %f", evaluated[0].Volume24h)
		}
	})
}
